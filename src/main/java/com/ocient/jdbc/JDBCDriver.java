...
public class JDBCDriver implements Driver {
  ...
  private Connection createConnection(
      final String hostname, final int portNum, final String database, final Properties properties)
      throws SQLException {
    ...
    try {
      ...
      LOGGER.log(
          Level.INFO,
          String.format("Received %d IP addresses for hostname %s", addrs.length, hostname));
      boolean connected = false;
      Throwable lastError = null;
      for (final InetAddress addr : addrs) {
        try {
          final String url =
              "jdbc:ocient://" + hostname + ":" + Integer.toString(portNum) + "/" + database;
          conn =
              new XGConnection(
                  user, pwd, addr.getHostAddress(), portNum, url, database, version, force, tls);
          ...
          LOGGER.log(
              Level.WARNING,
              String.format(
                  "Failed connecting to %s with exception %s with message %s",
                  addr.toString(), e.toString(), e.getMessage()));
        }
      }
      ...
      final Exception connInfo =
          new Exception(
              "Connection failed connecting to "
                  + hostname
                  + ":"
                  + portNum
                  + " - "
    ...
    synchronized (logMonitor) {
      ...
      try {
        ...
        LOGGER.log(
            Level.INFO,
            String.format(
                "Enabling logger with jdbc jar version: %s",
